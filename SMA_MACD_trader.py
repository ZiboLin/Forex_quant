...
class SMA_MACD_Trader(tpqoa.tpqoa):
    def __init__(self, conf_file, instrument, bar_length, units, SMA_S = 10, SMA_L = 30, MACD_S =12, MACD_L = 26):
    ...
    def define_strategy(self): # "strategy-specific"
        ...
        df["SMA_position"] = np.where(df["SMA_S"] > df["SMA_L"], 1, -1)

        df["MACD_short"] = df[self.instrument].ewm(span = self.MACD_S, adjust = False).mean()
        df["MACD_long"] = df[self.instrument].ewm(span = self.MACD_L, adjust = False).mean()
        df["MACD"] = df["MACD_short"] - df["MACD_long"]
        df["MACD_position"] = np.where(df["MACD"]>0,1,-1)

        df["position"] = np.where(df["MACD_position"] == df["SMA_position"],df["SMA_position"],np.nan)
        #***********************************************************************
...
if __name__ == "__main__": 
	...
	#initilise the class 
	trader = SMA_MACD_Trader("oanda.cfg", "EUR_USD", bar_length = "20min",
	                   SMA_S = 10, SMA_L = 30, MACD_S =12, MACD_L = 26, units = 100000)
