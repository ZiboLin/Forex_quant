#Mechine learning with mutiple index
...
class MLBacktester():
    def __init__(self,symbol,start,end,granularity,tc = 0.00007, lags =5, shift = 1,SMA_short = 5, SMA_long = 20, train_ratio = 0.7):
        self.symbol = symbol
        self.start = start 
        self.end = end
        self.shift = shift
    ...
    def get_data(self):
        ...
        raw["SMA"] = raw["price"].rolling(self.SMA_short).mean()

        ####### multiple features (returns,MACD,SMA=5,SMA=20)
        #### EMA must be used 
        ...
        raw["SMA5"] = raw["price"].rolling(self.SMA_short).mean()
        raw["SMA20"] = raw["price"].rolling(self.SMA_long).mean()
    ...
    def prepare_features(self, start, end):
        ...
        # self.data_subset["returns_1"] = self.data["returns"].shift(1)
        # self.data_subset["MACD_1"] = self.data["MACD"].shift(1)
        # self.data_subset["SMA5_1"] = self.data["SMA5"].shift(1)
        # self.data_subset["SMA20_1"] = self.data["SMA20"].shift(1)


        # self.feature_columns.append("returns_1")
        # self.feature_columns.append("MACD_1")
        # self.feature_columns.append("SMA5_1")
        # self.feature_columns.append("SMA20_1")
